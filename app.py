...
def check_login():
  if "user_email" in st.session_state:
    if st.session_state["user_email"].endswith("@nhecotech.com"):
      ...
      st.error(
          f"❌ Acesso negado: {st.session_state['user_email']} não autorizado."
      )
      return False

  st.markdown("### 🔒 Acesso Restrito")
  st.markdown(
      "Faça login com sua conta corporativa `@nhecotech.com` para visualizar o"
      " dashboard."
  )
...
ano_selecionado = st.sidebar.multiselect(
    "Ano Base", sorted(df["Ano Base"].dropna().astype(str).unique())
)
uf_selecionada = st.sidebar.multiselect(
    "Estado (UF)", sorted(df["UF"].dropna().unique())
)
material_selecionado = st.sidebar.multiselect(
    "Material", sorted(df["material"].dropna().unique())
)

df_filtrado = df.copy()
if ano_selecionado:
  df_filtrado = df_filtrado[
      df_filtrado["Ano Base"].astype(str).isin(ano_selecionado)
  ]
if uf_selecionada:
...
total_compensada = (
    df_filtrado["Compensada"].sum() if "Compensada" in df_filtrado.columns else 0
)
total_em_aberto = (
    df_filtrado["Em Aberto"].sum() if "Em Aberto" in df_filtrado.columns else 0
)
total_projetada = (
    df_filtrado["Projetada"].sum() if "Projetada" in df_filtrado.columns else 0
)

soma_demanda_calc = total_compensada + total_em_aberto + total_projetada
perc_compensada = (
    (total_compensada / soma_demanda_calc * 100) if soma_demanda_calc > 0 else 0
)
perc_em_aberto = (
    (total_em_aberto / soma_demanda_calc * 100) if soma_demanda_calc > 0 else 0
)
perc_projetada = (
    (total_projetada / soma_demanda_calc * 100) if soma_demanda_calc > 0 else 0
)

kpi1, kpi2, kpi3, kpi4 = st.columns(4)
kpi1.metric(
    "Demanda Total (Calculada)", f"{soma_demanda_calc:,.0f}".replace(",", ".")
)
kpi2.metric(
    "Compensada",
    f"{total_compensada:,.0f}".replace(",", "."),
    f"{perc_compensada:.1f}%",
    delta_color="off",
)
kpi3.metric(
    "Em Aberto",
    f"{total_em_aberto:,.0f}".replace(",", "."),
    f"{perc_em_aberto:.1f}%",
    delta_color="off",
)
kpi4.metric(
    "Projetada",
    f"{total_projetada:,.0f}".replace(",", "."),
    f"{perc_projetada:.1f}%",
    delta_color="off",
)
...
with col_graf1:
  df_uf = (
      df_filtrado.groupby("UF", as_index=False)["DEMANDA TOTAL"]
      .sum()
...
with col_graf2:
  df_mat = (
      df_filtrado.groupby("material", as_index=False)["DEMANDA TOTAL"].sum()
  )
  fig_mat = px.pie(
      df_mat,
      values="DEMANDA TOTAL",
      names="material",
      title="Distribuição da Demanda por Material",
      hole=0.4,
  )
...
if demandas_grafico:
  ...
  # --- GRÁFICO 1: COMPOSIÇÃO POR UF (COLUNAS VERTICAIS EMPILHADAS) ---
  df_composicao_uf = df_filtrado.groupby("UF", as_index=False)[
      opcoes_demanda
  ].sum()
  df_composicao_uf["UF"] = df_composicao_uf["UF"].astype(str)

  df_uf_melted = df_composicao_uf.melt(
      id_vars=["UF"],
      value_vars=demandas_grafico,
      var_name="Tipo de Demanda",
      value_name="Valor",
  )
  ...
  # --- GRÁFICO 2: COMPOSIÇÃO POR SKU (BARRAS HORIZONTAIS EMPILHADAS - UM EMBAIXO DO OUTRO) ---
  df_composicao_sku = df_filtrado.groupby("SKU", as_index=False)[
      opcoes_demanda
  ].sum()
  df_composicao_sku["Total_Selecionado"] = df_composicao_sku[
      demandas_grafico
  ].sum(axis=1)
  df_composicao_sku = df_composicao_sku.sort_values(
      by="Total_Selecionado", ascending=False
  )
  ...
  if len(df_composicao_sku) > 20:
    ...
    dict_demais = {
        "SKU": "Demais SKUs",
        "Total_Selecionado": df_demais["Total_Selecionado"].sum(),
    }
  ...
  df_sku_melted = df_final_sku.melt(
      id_vars=["SKU"],
      value_vars=demandas_grafico,
      var_name="Tipo de Demanda",
      value_name="Valor",
...
with st.expander("Ver Dados Detalhados"):
  ...
  soma_linha = (
      df_tabela["Compensada"] + df_tabela["Em Aberto"] + df_tabela["Projetada"]
  )
  ...
  colunas_presentes = [
      col for col in COLUNAS_NUMERICAS if col in df_tabela.columns
  ]
  ...
  colunas_exibicao = [
      c for c in df_tabela.columns if not c.startswith("%")
